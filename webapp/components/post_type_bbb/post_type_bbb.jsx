	...
	render() {
		...
		if (props.meeting_status === 'STARTED') {
			...
			content = (<div onMouseEnter={this.getAttendees}>
				...
				<span>
					...
					</a>
					{
						...
						<a className="btn btn-lg btn-link" style={style.buttonEnd} onClick={this.endMeeting} href="#">
			...
			if (props.attendants == undefined || props.attendents === '') {
				...
				const attendants = props.attendants.split(',');
				attendees = '';
				attendeesFull = '';
				otherWords = '';
				...
				if (attendants != null && attendants.length > 0) {
					for (var i = 0; i < attendants.length; i++) {
						...
						attendeesFull += attendants[i];
						if (i != attendants.length - 1) {
							attendeesFull += ', ';
						}
					}
					if (attendants.length > 4) {
						otherWords = 'and ' + (
							attendants.length - 4) + ' others';
					}
				} else {
					attendees = 'there were no attendees in this session';
				}
						...
						<div>
							{
								props.is_published === 'true'
									? <a href={props.recording_url} rel="noopener noreferrer" target="_blank">
						...
						<div>
							...
							<div style={style.recordingBody}>
								<div>
									{
										props.is_published === 'true'
											? <a href={props.notes_url} rel="noopener noreferrer" target="_blank">
												{'Click to view notes'}
											</a>
											: <span style={style.summaryItemGreyItalics}>
